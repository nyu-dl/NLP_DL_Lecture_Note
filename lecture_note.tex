...
\subsubsection{Simple Recurrent Units}

So far, we have considered the simple transformation layer from
Eq.~\eqref{eq:rnn_layer}:
\begin{align*}
    f(x_t, \vh_{t-1}) = g(\mW \phi(x_t) + \mU \vh_{t-1}),
\end{align*}
...
and maxout, has become standard. The main difference between logistic functions
...
\begin{align}
    ...
    \mW\phi(x_{t'})}_{(a)} + O(\epsilon),
\end{align}
where $x_{t'}$ is the last non-zero symbol in the input sequence, and
$O(\epsilon)$ is an error in the approximation.
...
$\mL$ and $\mR$ are orthogonal matrices. Then
\begin{align*}
...
\subsubsection{Gated Recurrent Units: Simple Recurrent Neural Networks are {\em Unrealistic}}
...
Now there remains a big question: {\em How are the update $\vu$ and reset $\vr$ gates
computed?}
